...
} from "@/app/utils/search-history";
import { useState, useEffect, useContext } from "react";

import { MyContext } from "@/app/context/context";

export default function UserSearchHistory({ onHistoryClick }) {
	...
	return (
		isLoggedIn && (
			<div className="mb-4">
				<p className="small font-bold tracking-tight font-gta pl-3 mb-2">
					Recent searches
				</p>{" "}
						...
						<div className="divide-y divide-gray-200">
							{history.map((country, index) => (
								<button
									type="button"
									key={index}
									onClick={() => onHistoryClick(country.country_searched)}
									className="w-full flex items-center gap-2 p-3 hover:bg-gray-100 focus:bg-gray-100 text-left text-sm text-gray-600 transition-colors"
								>
									<svg
										className="w-4 h-4 text-gray-400 flex-shrink-0"
										viewBox="-8.16 -8.16 40.32 40.32"
										fill="none"
